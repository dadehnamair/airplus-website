...
export const SCENES = ['none', 'tickets', 'towers', 'airport'];
export const CTA_SCENES = ['none', 'airport'];
...
export function validateContent(d) {
  ...
  for (let i = 0; i < d.sections.length; i++) {
    ...
    if (s.type === 'cta' && s.scene !== undefined && !CTA_SCENES.includes(s.scene)) return `${where}: صحنه نامعتبر`;
  }
