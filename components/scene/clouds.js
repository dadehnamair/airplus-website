  ...
  for (let tile = 0; tile < 4; tile++) {
    ...
    for (let py = 0; py < T; py++) {
      for (let px = 0; px < T; px++) {
        ...
        const lum = (0.78 + 0.22 * top) * (0.9 + 0.1 * n);
        const i = (py * T + px) * 4;
...
void main(){
  ...
  if (a < 0.003) discard;
  float shade = uToneAlpha > 0.5 ? 1.0 : (0.78 + 0.22 * vTone);
  vec3 col = tint * shade * (0.78 + 0.22 * t.r);
...
export function createCloudField({ atlas, curve, L, rand, banks = [], runwayT = 0.9 }) {
  ...
  function cluster(cx, cy, cz, r, count, sizeMul = 1, flat = 0.7, toneBoost = 0) {
    for (let k = 0; k < count; k++) {
      ...
      const tone = Math.min(1, Math.max(0.7, 0.85 + rel * 0.15 + (rand() - 0.5) * 0.1 + toneBoost));
